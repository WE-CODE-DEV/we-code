...
<body>
    <div class="container">
        <div class="search-container">
            <div class="search">
                <label>
                    <input type="text" placeholder="Search..." id="search-input" role="searchbox" aria-label="Search input">
                    <button id="clear-search" aria-label="Clear search">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="m16 8-8 8m4-4 4 4M8 8l2 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </button>
            ...
            <div class="search-results" aria-live="polite" role="list" aria-label="Search results">
                <ul></ul>
            </div>
        </div>
    </div>
    <template id="loading-temp">
    ...
    <template id="search-result-temp">
        <li>
...
const loadingTemp = document.getElementById('loading-temp');
const searchResultTemp = document.getElementById('search-result-temp');
const clearSearchBtn = document.getElementById('clear-search');

let debounceTimeout;

searchResultsContainer.addEventListener('animationend', (event) => {
    if(event.animationName === 'flip-back'){
        searchResultsContainer.querySelector('ul').innerHTML = '';
    }
});
...
clearSearchBtn.addEventListener('click', () => {
    searchInput.value = '';
    searchInput.focus();
    searchContainer.setAttribute('data-state', 'stop-searching');
});

searchInput.addEventListener('input', () => {
    clearTimeout(debounceTimeout);
    debounceTimeout = setTimeout(handleSearchInput, 500);
});
...
const showLoading = () => {            
    ...
    const loadingCnt = loadingTemp.cloneNode(true).content;
    const liEle = document.createElement('li');

    liEle.classList.add('loading-li');
    liEle.appendChild(loadingCnt);
    loadingFragment.appendChild(liEle);
    searchResults.appendChild(loadingFragment);
}

const showingResponse = (typedValue) => {
    const searchResults = document.querySelector('.search-results ul');
    ...
    searchContainer.setAttribute('data-state', 'result-found');

    if(matchFound){

        const searchResult = searchResultTemp.cloneNode(true).content;
        const startIndex = defaultTxt.toLowerCase().indexOf(typedValue.toLowerCase());
        const endIndex = startIndex + typedValue.length;
        const pTag = \`<p>
                        \${defaultTxt.slice(0, startIndex)}<span>\${defaultTxt.slice(startIndex, endIndex)}</span>\${defaultTxt.slice(endIndex)}
                    </p>\`;

        searchResult.querySelector('.content').insertAdjacentHTML('afterbegin', pTag);

        searchResults.appendChild(searchResult);
    }
    else{
        const liEle = document.createElement('li');
        liEle.textContent = 'No results found..';
        searchResults.appendChild(liEle);
    }
}
...
function handleSearchInput(){
    ...
    searchContainer.setAttribute('data-state', isValid ? 'searching' : 'stop-searching');

    if(!isValid) return;

    showLoading();

    const loadingTimeDelay = setTimeout(() => {
